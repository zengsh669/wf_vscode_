-- Validation query for SILVER.dbo.Arrears_Report
-- Run this to preview the SP output without writing to the table
...
SELECT
    ...
    [Date Paid To],
    [Security Level],
    CAST(person_id     AS NVARCHAR(50))  AS person_id,
    CAST(relationship  AS NVARCHAR(10))  AS relationship,
    Branch,
    FixCode,
    Product_Description,
    ...
    GroupingDesc,
    billing_type,
    CAST(billing_freq  AS NVARCHAR(10))  AS billing_freq,
    CAST([Tpt Period]  AS NVARCHAR(10))  AS [Tpt Period],
    AgentDesc,
    [Postal Preference],
    [Main Ref Type],
    BillingDes,
    [Payment Type],
    typebill,
    CAST(amount        AS NVARCHAR(10))  AS amount,
    tpt_day,
    Directdebitfilter,
    CAST([DaysInArrears] AS NVARCHAR(10)) AS [DaysInArrears],
    [Run Date],
    [Run Month],
    [Arrears Category]
FROM Bucket3;
