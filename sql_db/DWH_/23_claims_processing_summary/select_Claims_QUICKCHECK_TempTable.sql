-- =============================================================================
-- select_Claims_QUICKCHECK_TempTable.sql
-- *** ONE-OFF SCRIPT -- NOT the deliverable, NOT a faithful Claims translation. ***
--
-- Same 11 dashboard numbers as select_Claims_QUICKCHECK.sql, but split into two
-- parts run in the SAME SSMS query window/session:
--   Part 1: materialise the "heavy" 21-BRONZE-table join chain (ClaimStatusMain
--           through Final) into a local temp table, #Claims_Final_Snapshot.
--   Part 2: compute the 11 numbers as simple aggregates against that temp
--           table instead of re-running the full join chain for every
--           subquery (select_Claims_QUICKCHECK.sql's 11 numbers were each a
--           scalar subquery over the `Final` CTE, which the optimiser is not
--           guaranteed to only evaluate once).
--
...
--
-- IMPORTANT: #Claims_Final_Snapshot only exists for the lifetime of THIS SSMS
-- query window/session. Run Part 1 and Part 2 together (or Part 1 first, then
-- Part 2, without closing the window or losing the connection) -- a different
-- query window will not see it. If you need it to survive across sessions or
-- days, this approach won't work -- see the permission-based alternatives
-- instead (grant CREATE TABLE somewhere, or have someone pre-create a real
-- table for INSERT INTO).
-- =============================================================================
